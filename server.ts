...
// Helper functions for secure input validation to prevent injection, diagnostic details leakage, and DoS
function validateString(val: any, name: string, minLen = 1, maxLen = 5000): string {
  if (typeof val !== "string") {
    throw new Error(`Invalid ${name}: must be a string`);
  }
  const trimmed = val.trim();
  if (trimmed.length < minLen || trimmed.length > maxLen) {
    throw new Error(`Invalid ${name} length: must be between ${minLen} and ${maxLen} characters`);
  }
  return trimmed;
}

function validateInteger(val: any, name: string, minVal = -Infinity, maxVal = Infinity): number {
  ...
  if (parsed < minVal || parsed > maxVal) {
    throw new Error(`Invalid ${name} bounds: must be between ${minVal} and ${maxVal}`);
  }
  return parsed;
}
...
function validateHistory(history: any): ChatHistoryItem[] {
  ...
  if (history.length > 50) {
    throw new Error("Invalid history length: exceeded limit");
  }
  return history.map((item: any, idx: number) => {
    if (typeof item !== "object" || item === null) {
      throw new Error(`Invalid history item at index ${idx}: must be an object`);
    }
    const role = validateString(item.role, `history[${idx}].role`, 1, 50);
    const content = validateString(item.content, `history[${idx}].content`, 1, 5000);
    return { role, content };
  });
}
...
4. Provide a pristine point-based marking scheme, acceptable alternate answers, examiner's notes, model answers, user-friendly simple explanations, and a list of common errors.`;
    ...
    const isValidationError = error.message && error.message.startsWith("Invalid ");
    if (isValidationError) {
      return res.status(400).json({ error: error.message });
    }
    console.error("AI Generation error:", error);
    res.status(500).json({ error: "Failed to generate question: An unexpected system error occurred." });
  }
});
...
app.post("/api/ai/tutor", async (req, res) => {
  try {
    const message = validateString(req.body.message, "message", 1, 5000);
    const validatedHistory = validateHistory(req.body.history);
    ...
    const chatHistory = validatedHistory.map((h: ChatHistoryItem) => ({
    ...
    res.status(500).json({ error: "Tutor offline: An unexpected system error occurred." });
  }
});
...
app.post("/api/ai/reading-assistance", async (req, res) => {
  try {
    const text = validateString(req.body.text, "text", 1, 20000);
...
---`;
    ...
    res.status(500).json({ error: "Failed to generate reading assistance: An unexpected system error occurred." });
  }
});
...
app.post("/api/ai/online-igcse-material", async (req, res) => {
  try {
    const gradeNum = validateInteger(req.body.grade ?? 7, "grade", 1, 11);
    const subName = validateString(req.body.subject ?? "General Science", "subject", 1, 100);
    const topic = validateString(req.body.topicKeyword ?? "Core Concepts", "topicKeyword", 1, 200);
...
Grade Level: Grade ${gradeNum} (${curriculumStage})
Subject: ${subName}
...
7. Generate 3 or 4 relevant academic hashtag keywords.`;
    ...
    res.status(500).json({ error: "Failed to synthesize approved online study material: An unexpected system error occurred." });
  }
});
