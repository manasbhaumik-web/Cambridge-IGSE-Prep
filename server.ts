...
// Custom error for validation errors (returning HTTP 400)
export class ValidationError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "ValidationError";
    // Set prototype explicitly for built-in classes in ES5/ES6 environments
    Object.setPrototypeOf(this, ValidationError.prototype);
  }
}

// Input validation helper for strings with type and length bounds checking
export function validateString(
  ...
  if (trimmed.length < minLength) {
    throw new ValidationError(`${name} must be at least ${minLength} character(s) long.`);
  }
...
// Input validation helper for integers with type, parsing, and bounds checking
...
app.post("/api/ai/generate-question", async (req, res) => {
  try {
    ...
    // Validate inputs
    const validatedSubject = validateString(subject, "subject", 1, 100);
    const validatedTopic = validateString(topic, "topic", 1, 200);
    const validatedDifficulty = validateString(difficulty, "difficulty", 1, 50);
    const validatedPaperType = validateString(paperType, "paperType", 1, 100);
...
4. Provide a pristine point-based marking scheme, acceptable alternate answers, examiner's notes, model answers, user-friendly simple explanations, and a list of common errors.`;
    ...
    res.status(500).json({ error: "Failed to generate question: An unexpected internal server error occurred." });
  }
});
...
app.post("/api/ai/tutor", async (req, res) => {
  try {
    ...
    // Validate inputs
    const validatedMessage = validateString(message, "message", 1, 2000);
    ...
    if (history !== undefined && history !== null) {
      ...
      for (let i = 0; i < history.length; i++) {
        ...
        const role = validateString(h.role, `history[${i}].role`, 1, 50);
        if (role !== "user" && role !== "model" && role !== "assistant") {
          throw new ValidationError(`history[${i}].role must be 'user', 'model', or 'assistant'.`);
        }
        const content = validateString(h.content, `history[${i}].content`, 1, 4000);
    ...
    const chatHistory = validatedHistory.map((h: { role: string; content: string }) => ({
    ...
    res.status(500).json({ error: "Tutor offline: An unexpected internal server error occurred." });
  }
});
...
app.post("/api/ai/reading-assistance", async (req, res) => {
  try {
    ...
    // Validate inputs
    const validatedText = validateString(text, "text", 1, 10000);
    const validatedNumQuestions = validateInteger(numQuestions === undefined ? 3 : numQuestions, "numQuestions", 1, 10);
...
---`;
    ...
    res.status(500).json({ error: "Failed to generate reading assistance: An unexpected internal server error occurred." });
  }
});
...
app.post("/api/ai/online-igcse-material", async (req, res) => {
  try {
    ...
    // Validate inputs
    const validatedGrade = validateInteger(grade, "grade", 1, 11);
    const validatedSubject = validateString(subject, "subject", 1, 100);
    const validatedTopicKeyword = validateString(topicKeyword, "topicKeyword", 0, 200, true);
...
7. Generate 3 or 4 relevant academic hashtag keywords.`;
    ...
    res.status(500).json({ error: "Failed to synthesize approved online study material: An unexpected internal server error occurred." });
  }
});
