...
export function validateHistory(history: any): Array<{ role: string; content: string }> {
  ...
  for (let i = 0; i < history.length; i++) {
    ...
    if (typeof role !== "string" || (role !== "user" && role !== "model" && role !== "assistant" && role !== "system")) {
      throw new ValidationError(`history[${i}].role must be one of: user, model, assistant, system.`);
    }
    if (typeof content !== "string") {
      throw new ValidationError(`history[${i}].content must be a string.`);
    }
    if (content.length > 5000) {
      throw new ValidationError(`history[${i}].content cannot exceed 5000 characters.`);
    }
...
app.post("/api/ai/generate-question", async (req, res) => {
  try {
    const subject = validateString(req.body.subject, "subject", 1, 500);
    const topic = validateString(req.body.topic, "topic", 1, 500);
    const difficulty = validateString(req.body.difficulty, "difficulty", 1, 500);
    const paperType = validateString(req.body.paperType, "paperType", 1, 500);
...
4. Provide a pristine point-based marking scheme, acceptable alternate answers, examiner's notes, model answers, user-friendly simple explanations, and a list of common errors.`;
    ...
    res.status(500).json({ error: "An unexpected backend error occurred. Please try again later." });
  }
});
...
app.post("/api/ai/online-igcse-material", async (req, res) => {
  try {
    const gradeNum = validateInteger(req.body.grade, "grade", 1, 11);
    const subName = validateString(req.body.subject, "subject", 1, 500);
    const validatedTopicKeyword = validateString(req.body.topicKeyword !== undefined && req.body.topicKeyword !== null ? req.body.topicKeyword : "Core Concepts", "topicKeyword", 0, 500);
