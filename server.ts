...
// Custom ValidationError for precise identification of client validation issues
...
function validateString(value: any, name: string, minLength = 1, maxLength = 2000): string {
  ...
  if (trimmed.length > maxLength) {
    throw new ValidationError(`Parameter '${name}' must not exceed ${maxLength} characters.`);
  }
  return trimmed;
}
...
function validateInteger(value: any, name: string, min?: number, max?: number): number {
  ...
  if (isNaN(parsed) || !Number.isInteger(parsed)) {
    throw new ValidationError(`Parameter '${name}' must be a valid integer.`);
  }

  if (min !== undefined && parsed < min) {
    throw new ValidationError(`Parameter '${name}' must be at least ${min}.`);
  }
  if (max !== undefined && parsed > max) {
    throw new ValidationError(`Parameter '${name}' must be at most ${max}.`);
  }
  return parsed;
}
...
app.post("/api/ai/generate-question", async (req, res) => {
  try {
    ...
    const topic = validateString(req.body.topic, "topic", 1, 300);
...
4. Provide a pristine point-based marking scheme, acceptable alternate answers, examiner's notes, model answers, user-friendly simple explanations, and a list of common errors.`;
    ...
    res.status(500).json({ error: "Failed to generate question due to an unexpected backend error." });
  }
});
...
app.post("/api/ai/tutor", async (req, res) => {
  try {
    const message = validateString(req.body.message, "message", 1, 10000);
    const history = req.body.history;
    if (history !== undefined) {
      if (!Array.isArray(history)) {
        throw new ValidationError("Parameter 'history' must be an array.");
      }
      for (const h of history) {
        if (typeof h !== "object" || h === null) {
          throw new ValidationError("Each item in 'history' must be an object.");
        }
        validateString(h.role, "history[].role", 1, 50);
        validateString(h.content, "history[].content", 1, 10000);
      }
    }
    ...
    res.status(500).json({ error: "Tutor offline due to an unexpected backend error." });
  }
});
...
app.post("/api/ai/reading-assistance", async (req, res) => {
  try {
    const text = validateString(req.body.text, "text", 1, 50000);
...
---`;
    ...
    res.status(500).json({ error: "Failed to generate reading assistance due to an unexpected backend error." });
  }
});
...
app.post("/api/ai/online-igcse-material", async (req, res) => {
  try {
    ...
    const topicKeyword = req.body.topicKeyword !== undefined ? validateString(req.body.topicKeyword, "topicKeyword", 0, 200) : "Core Concepts";
...
7. Generate 3 or 4 relevant academic hashtag keywords.`;
    ...
    res.status(500).json({ error: "Failed to synthesize approved online study material due to an unexpected backend error." });
  }
});
